...
double search_r_golden(const std::string& type, const std::string& approach, double gammamin, double gammamax, double &optimgamma, double tol, double asn, double m, double tau, double p, double epsilon, double deltamin, double deltamax, double B[], double R[], double N[], double &GG, double &HH) {
  ...
  for(int j = 0; j < nn-1; ++j){
    cout << "a = " << a << ", b = " << b << "; c = " << c << ", fc = " << fc << "; d = " << d << ", fd = " << fd << endl;
    if(fc < fd){
      b = d;
      d = c;
      fd = fc;
...
// [[Rcpp::export]]
NumericVector AANOSgHgenerator(List type, List approach, double gamma, double asn, double m, double tau) {
  double B[M], R[M*M], N[M];
  NumericVector vec(3);
  double g, h, EAANOSs, p, epsilon, deltamin, deltamax;
  const std::string method = approach["approach"];
  const std::string objfunc = type["type"];
  if(method == "GICP"){
    p = approach["p"];
  ...
  EAANOSs = AANOSgh_generator(objfunc, method, g, h, gamma, asn, m, tau, p, epsilon, deltamin, deltamax, B, R, N);
  vec(0) = EAANOSs;
  vec(1) = g;
  vec(2) = h;
  return vec;
