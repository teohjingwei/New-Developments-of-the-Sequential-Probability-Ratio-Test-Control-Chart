...
/***
 ...
 // [[Rcpp::export]]
 NumericVector ANOSgh_generator(const std::string& type, int method, double k, double gamma, double n, double RATE, double tau, double deltamumin, double deltamumax, double deltasigmamin, double deltasigmamax)
 {
 double C[M], R[M*M], N[M];
 NumericVector vec(3);
 double g, h;
 
 vec(0) = ANOSgh_generator(type, method, g, h, k, gamma, n, RATE, tau, deltamumin, deltamumax, deltasigmamin, deltasigmamax, C, R, N);
 vec(1) = g;
 vec(2) = h;
