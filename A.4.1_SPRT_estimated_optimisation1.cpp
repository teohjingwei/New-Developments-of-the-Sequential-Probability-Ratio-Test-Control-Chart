...
double search_nr_golden(const std::string& type, const std::string& approach, double gammamin, double gammamax, double asnmin, double asnmax, double &optimasn, double &optimgamma, double gammatol, double asntol, double m, double RATE, double tau, double p, double epsilon, double deltamin, double deltamax, double B[], double R[], double N[], double &GG, double &HH) {
  ...
  if(mm == nn){
    for(int j = 0; j < nn-1; ++j){
      ...
      cout << "gamma1: " << gamma1 << ", gamma2: " << gamma2 << ", asn1: " << asn1 << ", asn2: " << asn2 << endl;
    }
  }else if(mm < nn){
...
// [[Rcpp::export]]
NumericVector AAEQLdgHgenerator(List type, List approach, double gamma, double asn, double m, double RATE, double tau) {
  double B[M], R[M*M], N[M];
  NumericVector vec(4);
  double g, h, AAEQLs, p, epsilon, deltamin, deltamax;
  const std::string method = approach["approach"];
  const std::string objfunc = type["type"];
  if(method == "GICP"){
    p = approach["p"];
  ...
  AAEQLs = AAEQLdgh_generator(objfunc, method, g, h, gamma, asn, m, RATE, tau, p, epsilon, deltamin, deltamax, B, R, N);
  vec(0) = AAEQLs;
  vec(1) = asn/RATE;
  vec(2) = g;
  vec(3) = h;
